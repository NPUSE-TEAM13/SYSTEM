...
<body class="layui-anim layui-anim-up">
			...
			<tbody>
				<c:forEach items="${staffList }" var="list">
	              ...
	              <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'><i class="layui-icon">&#xe605;</i></div>
						...
						<td>${list.office.officeNumber }</td>
						<td>${list.office.officeKind }</td>
						<!--<td>${list.onJob }</td>  -->
						<c:if test="${list.onJob == true}">
							<td>在职</td>
						</c:if>
						<c:if test="${list.onJob == false}">
							<td>离职</td>
						</c:if>
						<td class="td-manage">
							<button class="layui-btn layui-btn layui-btn-xs"
								onclick=" member_edit('${list.staffID }','编辑','<%=request.getContextPath()%>/officeManage/checkEdit')">
	...
	<script>
      ...
      $("queryBtn").click(function () {
          ...
          $.ajax({
              url:"/officeManage/findStaffById",
      ...
      function queryData() {
    	  ...
    	  window.location.href = "/sisms/officeManage/showQueryStaff?queryData=" + queryData;
      ...
      /*用户-编辑*/
    	function member_edit(staffID,title,url,w,h){
    		...
    		var Date={
    	        ...
    	        $.ajax({
    	      	  type:"POST",
    	      	  url:"<%=request.getContextPath() %>/officeManage/checkEditHelp",
    	      	  data:Date,
    	      	  success:function(data){	
    	      	  }
    	        });
