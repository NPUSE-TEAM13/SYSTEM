        ...
        <tbody class="x-cate">
       	 <c:forEach items="${courseList }" var = "list">
				<tr cate-id='1' fid='0'>
					...
					<td>${list.length }</td>
					<!-- ${list.auditing } -->
					<c:if test="${list.auditing == true}"><td><input type="checkbox" name="switch" lay-text="通过|未过" checked="checked" lay-skin="switch" onclick="member_change(${list.courseNumber})" ></td></c:if>
					<c:if test="${list.auditing == false}"><td><input type="checkbox" name="switch" lay-text="通过|未过"  lay-skin="switch" onclick="member_change(${list.courseNumber})" ></td></c:if>
